...
def main():
    ...
    dataset = Image2ImageDatasetX2("dat/images_color_train.dat",root+"linex2/",root+"colorx2/", train=True)
...
class ganUpdater(chainer.training.StandardUpdater):
    ...
    def update_core(self):        
        ...
        for i in range(batchsize):
            ...
            for ch in range(3):
                color_ch = cv2.resize( batch[i][1][ch], (w_out,w_out), interpolation = cv2.INTER_CUBIC).astype("f")
        ...
        for j in range(batchsize):
            for ch in range(3):
                ...
                if np.random.rand() < 0.8:
                     x_in_2[j,1+ch,:] = xp.asarray( cv2.resize(x_out[j,ch,:], (w_in_2,w_in_2), interpolation = cv2.INTER_CUBIC) )
