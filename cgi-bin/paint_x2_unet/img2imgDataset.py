...
class ImageAndRefDataset(chainer.dataset.DatasetMixin):
    ...
    def get_example(self, i , minimize = False , blur=0, s_size=128):
        ...
        if minimize :
            ...
            _image1 = image1.copy() 
            _image1 = cv2.resize(_image1,(4*s1,4*s0),interpolation = cv2.INTER_AREA)
...
class Image2ImageDataset(chainer.dataset.DatasetMixin):
    ...
    def get_example(self, i , minimize = False , log = False , bin_r = 0 ):
        ...
        if self._train :
            noise = np.random.normal(0,5*np.random.rand(),image1.shape).astype(self._dtype) 
            image1 += noise
            noise = np.random.normal(0,5*np.random.rand(),image2.shape).astype(self._dtype) 
            image2 += noise
...
class Image2ImageDatasetX2(Image2ImageDataset):
    ...
    def get_example(self, i , minimize = False , log = False , bin_r = 0):
        ...
        if self._train :
            ...
            image2 = image2 + noise
            noise = np.random.normal(0,4*np.random.rand(),_image1.shape).astype(self._dtype) 
