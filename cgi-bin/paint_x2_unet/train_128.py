...
class ganUpdater(chainer.training.StandardUpdater):
    ...
    def update_core(self):        
        ...
        y_target = self.dis( x_out, Variable(xp.zeros(batchsize,dtype=np.int32)), test=False)

        cnn_optimizer.update(self.loss_cnn, self.cnn, x_out, t_out, y_target)
        
        x_out.unchain_backward()
        y_fake = self.dis( x_out,  Variable(xp.ones(batchsize,dtype=np.int32)), test=False)
        y_real = self.dis( t_out,  Variable(xp.zeros(batchsize,dtype=np.int32)),test=False)
