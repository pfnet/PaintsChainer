...
class MyHandler(http.server.CGIHTTPRequestHandler):
    ...
    def ret_result(self, success):
        if success:
            content = bytes(
                "{ 'message':'The command Completed Successfully' , 'Status':'200 OK','success':true }", "UTF-8")
            self.send_response(200)
        else:
            content = bytes(
                "{ 'message':'The command Failed' , 'Status':'503 NG','success':false }", "UTF-8")
...
parser = argparse.ArgumentParser(
    description='chainer line drawing colorization file post server')
